function out=GICA_surrogates(S,t,d,p,Fs,band_all,numsurr,perc_GC_GI,perc_GA,q,nfft)
...
out_dr=GICA_LinReg(S,d,[1 2],(1:p));
eAm_dr=out_dr.eA';
%eSu_dr=out_dr.es2u;
U_dr=out_dr.eu;
...
out_GA_targ=GICA_LinReg(S,t,d,(1:p)); % maintain the same model order 
...
out_GC_targ=GICA_LinReg(S,t,t,(1:p));
...
for k = 1:numsurr
    ...
    %%% GA surrogates
    S_surr_GA=GICA_MVARfilter(eAm_GA,U_surr_GA'); % same estimated AR coefficients, resampled residuals
    out_surro_GA_full=GICA_LinReg(S_surr_GA',[1 2],[1 2],(1:p)); % VAR model identification (full)
    eAm_GAsurr=out_surro_GA_full.eA';
    eSu_GAsurr=out_surro_GA_full.es2u;
    ret_GAsurr = GICA_computation(eAm_GAsurr,eSu_GAsurr,t,d,q,nfft,Fs); % compute measures for the surrogate    
    A_Y_surr(k)=ret_GAsurr.A_Y; %%% GA time domain
    a_Y_surr(:,k)=ret_GAsurr.a_Y_all;  %%% GA frequency domain
    
    %%% GC surrogates
    S_surr_GC=GICA_MVARfilter(eAm_GC,U_surr_GC'); % same estimated AR coefficients, resampled residuals
    out_surro_GC_full=GICA_LinReg(S_surr_GC',[1 2],[1 2],(1:p)); % VAR model identification (full)
    eAm_GCsurr=out_surro_GC_full.eA';
    eSu_GCsurr=out_surro_GC_full.es2u;
    ret_GCsurr = GICA_computation(eAm_GCsurr,eSu_GCsurr,t,d,q,nfft,Fs); % compute measures for the surrogate
